...
SELECT  nombre AS Nombre, cargo_emp AS Cargo FROM empleados ORDER BY sal_emp ASC;

#9. Elabore un listado donde para cada fila, figure el alias ‘Nombre’ y ‘Cargo’ para las respectivas tablas de empleados.
SELECT nombre AS Nombre, cargo_emp AS Cargo FROM empleados; #Consultar si esto es lo pedido
...
#11. Obtener el valor total a pagar que resulta de sumar el salario y la comisión de los empleados del departamento 3000 mas una bonificación de 500
# en orden alfabético del empleado.
SELECT nombre AS Nombre, (sal_emp + comision_emp + 500) AS TotalAPagar FROM Empleados WHERE id_depto = 3000 ORDER BY Nombre ASC;
...
SELECT sal_emp AS Salario, comision_emp AS Comision, (sal_emp + comision_emp) AS Total FROM empleados WHERE comision_emp > 1000;

#14. Obtener un listado similar al anterior, pero de aquellos empleados que NO tienen comisión.
SELECT sal_emp AS Salario, comision_emp AS Comision, (sal_emp + comision_emp) AS Total FROM empleados WHERE comision_emp = 0;

#15. Obtener la lista de los empleados que ganan una comisión superior a su sueldo.
SELECT nombre AS Nombre, sal_emp AS Salario, comision_emp AS Comision FROM empleados WHERE comision_emp > sal_emp;

#16. Listar los empleados cuya comisión es menor o igual que el 30% de su sueldo.
SELECT nombre AS Nombre, sal_emp AS Salario, comision_emp AS Comision FROM empleados WHERE comision_emp <= (sal_emp * 0.3);
...
SELECT nombre_depto FROM departamentos WHERE nombre_depto NOT IN ("Ventas", "Investigación","Mantenimiento") GROUP BY nombre_depto;
...
SELECT id_depto AS Departamento, COUNT(id_depto) AS CantidadEmpleados FROM empleados GROUP BY id_depto HAVING COUNT(id_depto) > 3; 
...
SELECT cod_jefe AS Jefe, COUNT(cod_jefe) AS CantidadEmpleados FROM empleados GROUP BY cod_jefe HAVING COUNT(cod_jefe) > 1;

#26. Hallar los departamentos que no tienen empleados
SELECT id_depto AS Departamento, COUNT(id_depto) AS CantidadEmpleados FROM empleados GROUP BY id_depto HAVING COUNT(id_depto) = 0;
...
SELECT nombre AS NombreEmpleado FROM empleados WHERE sal_emp >= (SELECT AVG(sal_emp) FROM empleados) ORDER BY id_depto ASC;
